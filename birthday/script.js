...
const gameBoard = $('#game-board'); const message = $('#game-message'); let score = 0; let gameTimer;
...
$('#start-game').addEventListener('click', (event) => { event.stopPropagation(); score = 0; $('#score').textContent = 0; message.remove(); $$('.game-star').forEach((star) => star.remove()); gameTimer = setInterval(spawnStar, 650); spawnStar(); });