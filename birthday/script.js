...
const spawnHazard = () => { const hazard = document.createElement('button'); hazard.className = 'game-hazard'; hazard.textContent = ['🪐', '🚀', '☄️'][Math.floor(Math.random() * 3)]; hazard.title = 'Oops! Lose a point'; hazard.style.left = `${8 + Math.random() * 84}%`; hazard.style.top = `${20 + Math.random() * 65}%`; hazard.addEventListener('click', () => { score = Math.max(0, score - 1); $('#score').textContent = score; hazard.remove(); }); gameBoard.appendChild(hazard); setTimeout(() => hazard.remove(), 2200); };
